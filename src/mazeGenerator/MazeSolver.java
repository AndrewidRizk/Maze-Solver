...
public class MazeSolver {
	 ...
	 * Note The first, second, third and fourth digits shows the state of the top, left, bottom and right
	 * 
	 */
	public boolean enoughGate () {
		...
		// 2D array because it is assumed that it is either a rectangular or a square 2D array
	...
	public boolean enoughGateHelper(char c, int pos, int gates, int length, int width)
	{
		...
		// Applying the necessary rotation such that it go though all the sides of the rectangular array 
		//		  a
		//	^----------->
		//	|			|
		//	|			|
	...
	public String findPath (int row, int column)  // returns --> shortest path
	{
		...
		// Checking the corners as they can have two walls open and can be treated as an entrance and an open path. in this case we class both walls 
		// calling the shortestPath method  to get the shortest path to the exit
		
		if(row == 0 && column == 0)
		{
			dogMaze.getReference()[row][column] = dogMaze.getReference()[row][column].substring(0, 0) + "1" + dogMaze.getReference()[row][column].substring(0+1); //closing the entrance so to mark it as a non possible solution
			dogMaze.getReference()[row][column] = dogMaze.getReference()[row][column].substring(0, 1) + "1" + dogMaze.getReference()[row][column].substring(1+1); //closing the entrance so to mark it as a non possible solution
			return shortestPath(findPathHelper('L', row, column, entranceRow,  entranceColumn, Path)); 
		}
		else if(row == 0 && column == width - 1)
		{
			dogMaze.getReference()[row][column] = dogMaze.getReference()[row][column].substring(0, 0) + "1" + dogMaze.getReference()[row][column].substring(0+1); //closing the entrance so to mark it as a non possible solution
			dogMaze.getReference()[row][column] = dogMaze.getReference()[row][column].substring(0, 3) + "1" + dogMaze.getReference()[row][column].substring(3+1); //closing the entrance so to mark it as a non possible solution
			return shortestPath(findPathHelper('L', row, column, entranceRow,  entranceColumn, Path)); 
		}
		else if(row == length - 1 && column == 0)
		{
			dogMaze.getReference()[row][column] = dogMaze.getReference()[row][column].substring(0, 1) + "1" + dogMaze.getReference()[row][column].substring(1+1); //closing the entrance so to mark it as a non possible solution
			dogMaze.getReference()[row][column] = dogMaze.getReference()[row][column].substring(0, 2) + "1" + dogMaze.getReference()[row][column].substring(2+1); //closing the entrance so to mark it as a non possible solution
			return shortestPath(findPathHelper('L', row, column, entranceRow,  entranceColumn, Path)); 
		}
...
/**
 * this method is a helper for the findPath. it checks if the block is on the side of the maze
 * and actually a gate
 ...
 * @return boolean if the block on a side of the sides of the 2D rectangular and it is a Gate returns true
 */
public boolean ifGate(int row, int column)
{
	...
	if((row == 0 && column == 0) && (dogMaze.getReference()[row][column].charAt(0) == '0' ||dogMaze.getReference()[row][column].charAt(1) == '0'))
	{
		return true;
	}
...
/**
 * this method is a helper for the method findPath. it takes in the full Path of the dog and returns the shortest path
 * the logic behind this is:
 * Case Zero: if the substring is empty just add the entrance of the maze
 * Case One: if the dog passed though a block only once and it have only one opening then it has to be in the shortest path
 * Case Two: If it is a gate (the exit) and not in the shortest Path already(just avoiding redundancy) then it has to be in the shortest path
 * Case Three: if the dog passed through a block more than one opening, and it is not already in the string and ( it is row is more than or less the previous
 * blocks row, and column is the same or the other way around) such lets say (1,2) the first one the then one after it has to be either 
 * (2,2) or (0,2) or (1,1) or (1,3) which either moves up or down or left or right then it has to be in the shortest path
...
public String shortestPath(String Path)
{
	...
	for(int i = 0 ; i < Path.length(); i = i+5)
	{
        ...
        else if(count<2 && !checkPath(Integer.parseInt(subString.substring(1, 2)), Integer.parseInt(subString.substring(3, 4)))  )
		{			
        	shortestPath = shortestPath + subString; 
        	previousSubString = subString; // saving the previous added substring 
		}
        else if(ifGate(Integer.parseInt(subString.substring(1, 2)), Integer.parseInt(subString.substring(3, 4))) && !shortestPath.contains(subString) )
        {			
        	shortestPath = shortestPath + subString;
        	previousSubString = subString; // saving the previous added substring 
		}
        else if(count>=1 && checkPath(Integer.parseInt(subString.substring(1, 2)), Integer.parseInt(subString.substring(3, 4))) && !shortestPath.contains(subString) && 
        		//making sure that the added substring in the right place, avoiding adding block that been went twice and not in the fastest path ( either moves up or down or left or right)
        		((Integer.parseInt(previousSubString.substring(1, 2)) == Integer.parseInt(subString.substring(1, 2)) - 1 && Integer.parseInt(previousSubString.substring(3, 4)) == Integer.parseInt(subString.substring(3, 4)))
        		|| ((Integer.parseInt(previousSubString.substring(1, 2)) == Integer.parseInt(subString.substring(1, 2)) + 1 && Integer.parseInt(previousSubString.substring(3, 4)) == Integer.parseInt(subString.substring(3, 4))))
        		|| ((Integer.parseInt(previousSubString.substring(3, 4)) == Integer.parseInt(subString.substring(3, 4)) - 1 && Integer.parseInt(previousSubString.substring(1, 2)) == Integer.parseInt(subString.substring(1, 2))))
        		|| ((Integer.parseInt(previousSubString.substring(3, 4)) == Integer.parseInt(subString.substring(3, 4)) + 1 && Integer.parseInt(previousSubString.substring(1, 2)) == Integer.parseInt(subString.substring(1, 2))))))
		{			
        	shortestPath = shortestPath + subString;
        	previousSubString = subString; // saving the previous added substring 
...
/**
 * this is a helper method that checks if a specific block have more than one opening or path
 ...
 * @param  c represents the previous move, basically where is the previous move coming from
 * 'T' --> Top
 * 'B'--> Bottom
 * 'L' --> Left
 * 'R' --> Right
 ...
 * @param Path that have all the movement of the dog tell he finds the  exit
 * @return the string of the right path to the exit including all the possible tries 
 */
public String findPathHelper(char c, int row, int column, int entranceRow, int entranceColumn, String Path)
{
	...
	{
		 Path = Path+ toString(row,column)  + findPathHelper('L', row, column + 1, entranceRow,  entranceColumn, Path); //--> going right
		return Path;
	}
	 if(c == 'T' && dogMaze.getReference()[row][column].charAt(2) == '0')
	{
		 Path =  Path +toString(row,column)  + findPathHelper('T', row + 1, column, entranceRow,  entranceColumn, Path); //--> going to the bottom
		return Path;
	}
	 if(c == 'T' && dogMaze.getReference()[row][column].charAt(1) == '0') 
	{
		 Path =  Path +toString(row,column)  + findPathHelper('R', row, column - 1, entranceRow,  entranceColumn, Path); //--> going to the left
		return Path;
	}
	...
	{
		 Path = Path + toString(row,column)  + findPathHelper('B', row - 1, column, entranceRow,  entranceColumn, Path); //--> going to the Top
		return Path;
	}
	...
	{
		 Path = Path+ toString(row,column)  + findPathHelper('T', row + 1, column, entranceRow,  entranceColumn, Path); //--> going Bottom
	...
	{
		 Path = Path +  toString(row,column) + findPathHelper('L', row, column + 1, entranceRow,  entranceColumn, Path); //--> going to the Right
			return Path;
	}
	else
	{
		if(c== 'T')
		{
			...
			return Path +  toString(row,column)  + findPathHelper('B', row, column, entranceRow,  entranceColumn, Path);
		}
		else if(c== 'B')
		{
			
				return Path +  toString(row,column)  + findPathHelper('T', row, column, entranceRow,  entranceColumn, Path);
		}
		else if(c== 'R')
		{
			
			return Path +  toString(row,column)  + findPathHelper('L', row, column, entranceRow,  entranceColumn, Path);
		}
		else if(c== 'L')
		{
			
			return Path +  toString(row,column)  + findPathHelper('R', row, column, entranceRow,  entranceColumn, Path);
		}
	}
		return "";
}
...
class Maze{
	...
	/**
	 * toString() method will print out the 2D string such that it prints every String in the 2D array
	 * one after another with a space " " in between, and an array after an array with a "]/n[" in 
	 * between.
	 * Note there is a "[" at the beginning and make an if-statement to delete the "[" at the end
	 * @return the String
	 */
